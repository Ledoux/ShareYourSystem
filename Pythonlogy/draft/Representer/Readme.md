...
def getRepresentedNumpyArray(_NumpyArray):
        ...
        #Return the array directly if it is small or either a short represented
version of it
        if (len(ShapeList)==1 and ShapeList[0]<3) or (len(ShapeList)>1 and
ShapeList[1]<3):
...
def getRepresentedPointerStrWithVariable(_Variable,**_KwargVariablesDict):
        ...
        RepresentedLocalAlineaStr=RepresentedAlineaStr if
_KwargVariablesDict['RepresentedDeepInt']==0 else ""
...
[RepresentingIndentStr]*(_KwargVariablesDict['RepresentedDeepInt']))
        ...
        if type(_DictatedVariable)==collections.OrderedDict:
                ...
                RepresentedTuplesList=sorted(
                        _DictatedVariable.iteritems(), key=lambda key_value:
key_value[0])

        #Integrativ loop for seriaizing the items
        for __RepresentedKeyStr,__RepresentedValueVariable in
...
RepresentedDictStr+="\n"+LocalRepresentedAlineaStr+RepresentingDictIndentStr
                ...
                if len(WordStrsList)>0:
                        ...
                        """
                        if
SYS.getWordStrsListWithStr(__RepresentedKeyStr)[-1]=="Pointer":
...
''.join(SYS.getWordStrsListWithStr(__RepresentedKeyStr)[-2:])=="PointersList":

                                #debug
                                '''
                                print('__RepresentedValueVariable is
',__RepresentedValueVariable)
                                print('')
                                '''
...
type(__RepresentedValueVariable)==list else "None"
                        """
                ...
                if RepresentedValueVariableStr=="None":
                        ...
                        print('__RepresentedKeyStr is ',__RepresentedKeyStr)
                        print('id(__RepresentedValueVariable) is
',id(__RepresentedValueVariable))
                        print('')
                        '''
...
RepresentedValueVariableStr=getRepresentedStrWithVariable(
                ...
                #Key and Value Case
                RepresentedDictStr+="'"+__RepresentedKeyStr+"' :
"+RepresentedValueVariableStr
...
ListedVariable,**dict(_KwargVariablesDict,**{'RepresentingAlineaIsBool':False})
                ...
                #Key and Value Case
                RepresentedListStr+=str(ListedVariableInt)+" :
"+RepresentedValueVariableStr
...
def getRepresentedStrWithVariable(_Variable,**_KwargVariablesDict):
        ...
        #print("hasattr(_Variable,'__repr__') is
"+str(hasattr(_Variable,"__repr__")))
        ##if hasattr(_Variable,"__repr__"):
        #       print('hasattr(_Variable.__class__,"InspectedOrderedDict") is
'+str(
...
']['KwargVariablesSetKeyStr'])
        ...
        if type(_Variable) in [dict,collections.OrderedDict]:
                ...
                print('RepresentedAlreadyIdIntsList is
',RepresentedAlreadyIdIntsList)
                print('')
                '''
...
',RepresentedAlreadyIdIntsList)
                        ...
                        #Return the repr of the _Variable but shifted with the
RepresentedAlineaStr
                        ...
                        RepresentedStr=RepresentingCircularStr+getRepresentedPoi
nterStrWithVariable(_Variable)
        ...
        #List types print
                ...
                if RepresentedIdInt not in RepresentedAlreadyIdIntsList:
                        ...
                        #debug
                        '''
                        print('This is a listed type so get a represent like a
list')
                        print('')
                        '''
                        ...
                        if all(
                                        map(
                                                ...
                                                type(ListedVariable) in
[float,int,str,unicode,numpy.float64] or ListedVariable==None,
                                                _Variable
                                                )
                                )==False:
                                ...
                                #Return
                                RepresentedStr=getRepresentedStrWithListedVariab
le(_Variable,**_KwargVariablesDict)
                                ...
                                #Definition the Local alinea
                                RepresentedLocalAlineaStr=RepresentedAlineaStr
if _KwargVariablesDict['RepresentedDeepInt']==0 else ""
...
_Variable).replace("\n","\n"+RepresentedLocalAlineaStr)
                        ...
                        #Return the circular Str
                        return
RepresentingCircularStr+getRepresentedPointerStrWithVariable(_Variable)
...
_KwargVariablesDict['RepresentedDeepInt']==0 else ""
                ...
                return
RepresentedLocalAlineaStr+_Variable.__repr__().split('of')[0]+">"
...
_KwargVariablesDict['RepresentedDeepInt']==0 else ""
                ...
                #Return
                return
RepresentedLocalAlineaStr+_Variable.replace("\n","\n"+RepresentedLocalAlineaStr)
...
and _Variable.__class__.InspectedArgumentDict[
                ...
                #debug
                '''
                print('This is a representer so call the repr of it with the
_KwargVariablesDict')
...
RepresentedAlreadyIdIntsList.append(RepresentedIdInt)
                        ...
                        #Return a repr of the _Variable but shifted with the
RepresentedAlineaStr
...
"\n"+RepresentedLocalAlineaStr
                        ...
                        #Return the circular Str
                        return RepresentedLocalAlineaStr+RepresentingCircularStr
+getRepresentedPointerStrWithVariable(
                                _Variable)
...
def represent(_Variable,**_KwargVariablesDict):
        ...
        if 'RepresentingAlineaIsBool' not in _KwargVariablesDict or
_KwargVariablesDict['RepresentingAlineaIsBool']:
                return
getRepresentedStrWithVariable(_Variable,**_KwargVariablesDict)
        else:
...
class RepresenterClass(BaseClass):
        ...
        def do_represent(self):
                ...
                #debug
                '''
                print('Representer l.352 : RepresentedClass is
',RepresentedClass)
                print('')
                '''
...
(
                        len(RepresentedClass.__bases__)>0 and
hasattr(RepresentedClass.__bases__[0
                        ],'RepresentingKeyStrsList') and
RepresentedClass.__bases__[0
...
RepresentedClass.RepresentedSpecificKeyStrsList
                        ...
                        if 'RepresentingBaseKeyStrsListBool' in
_KwargVariablesDict and _KwargVariablesDict['RepresentingBaseKeyStrsListBool']:
...
(
        ("<Spe><Instance>"
        if __NewItemTuple[0] in RepresentedClass.DefaultSetKeyStrsList+Represent
edClass.DefaultBaseSetKeyStrsList
...
_InstanceVariable.__dict__.items()
                        ...
                        if 'RepresentingNewInstanceKeyStrsListBool' not in
_KwargVariablesDict or _KwargVariablesDict[
                        'RepresentingNewInstanceKeyStrsListBool']:

                                RepresentedNewInstanceKeyStrsList=SYS._filter(
        lambda __NewItemTuple:
        __NewItemTuple[0] not in RepresentedClass.DefaultSetKeyStrsList+Represen
tedClass.DefaultBaseSetKeyStrsList,
        _InstanceVariable.__dict__.items()
)
        ...
        RepresentedNewInstanceKeyStrsList
                        ...
                        #Represent the NewClassKeyStrs in the
_RepresentedClass__.__dict__
                        if 'RepresentingNewClassKeyStrsListBool' not in
_KwargVariablesDict or _KwargVariablesDict[
        ...
        SYS._filter(
                                        lambda __KeyStr:
                                        __KeyStr not in
RepresentedClass.KeyStrsList and __KeyStr not in _InstanceVariable.__dict__,
                        ...
                        if 'RepresentingNotConcludeTuplesList' in
_KwargVariablesDict:
                                ...
                                RepresentedTuplesList=SYS._filter(
                                        ...
                                        any(
                                                map(
                                                        lambda
__RepresentingNotConcludeTuple:
...
_InstanceVariable.__dict__[__RepresentingKeyStr]
                                                ) if __RepresentingKeyStr in
_InstanceVariable.__dict__
                                                else (
...
_KwargVariablesDict['RepresentingKeyStrsList']
                        ...
                        print('_InstanceVariable not in
RepresentedAlreadyIdIntsList is ',str(
                                _InstanceVariable not in
RepresentedAlreadyIdIntsList))
                        print('')
                        '''
...
dict(RepresentedTuplesList),
                ...
                RepresentedClass.InspectedArgumentDict['__repr__']=SYS.getArgume
ntDictWithFunction(
...
<small>
View the Representer sources on <a href="https://github.com/Ledoux/ShareYourSyst
em/tree/master/Pythonlogy/ShareYourSystem/Classors/Representer"
...
#ImportModules
import ShareYourSystem as SYS
from ShareYourSystem.Standards.Classors import Representer,Attester
from ShareYourSystem.Standards.Objects import Initiator
...
SimpleMaker is < (MakerClass), 4537523152>
   /{
   /  '<New><Instance>IdInt' : 4537523152
   /  '<Spe><Class>MadeMyInt' : 0
   /}

------

ParentMaker is < (MakerClass), 4537521552>
   /{
   /  '<New><Instance>CircularChildMaker' : < (MakerClass), 4538290704>
   /   /{
   /   /  '<New><Instance>IdInt' : 4538290704
   /   /  '<New><Instance>ParentMaker' : {...}< (MakerClass), 4537521552>
   /   /  '<New><Instance>SelfMaker' : {...}< (MakerClass), 4538290704>
   /   /  '<Spe><Class>MadeMyInt' : 0
   /   /}
   /  '<New><Instance>FirstChildMaker' : < (MakerClass), 4538290512>
   /   /{
   /   /  '<New><Instance>IdInt' : 4538290512
   /   /  '<Spe><Class>MadeMyInt' : 0
   /   /}
   /  '<New><Instance>IdInt' : 4537521552
   /  '<Spe><Class>MadeMyInt' : 0
   /}

------

SimpleBuilder is < (BuilderClass), 4538290768>
   /{
   /  '<New><Instance>IdInt' : 4538290768
