...
class JoinerClass(BaseClass):
	...
	def model(self,**_KwargVariablesDict):
		...
		if self.JoinedDatabasePointer!=None:
			...
			#set the columns
			self.ModeledClass.columns[self.JoinedRetrievingIndexesListKeyStr]=tables.Int64Col(shape=2)
	...
	def row(self,**_VariablesList):
		...
		if self.JoinedDatabasePointer!=None:
			...
			#set the ModeledInt
			JoinedRetrievingIndexesList[1]=ModeledInt
	...
	def flushBefore(self,**_FlushingVariablesList):
		...
		if self.ModeledDict['JoiningModelStr']!="":
			...
			#Flush
			self.flush(self.ModeledDict['JoiningModelStr'])
	...
	def findBefore(self,**_FindingVariablesDict):
		...
		if JoiningModelStr!="":
			...
			#Make the TabledInt and the ModeledInt as a <JoiningModelStr>RetrievingIndexesList
			map(
